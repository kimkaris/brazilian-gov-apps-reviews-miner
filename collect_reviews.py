...
def save_to_split_csv(reviews, base_name="app_reviews"):
    max_rows = 1_000_000
    total = len(reviews)
    ...
    for i in range(parts):
        ...
        file_name = f"{base_name}_part_{i+1}.csv"
        part_df.to_csv(file_name, sep=";", index=False, columns=["app_id", "stars", "review"])
        print(f"File saved: {file_name} ({len(part_df)} reviews)")


try:
    # Collect reviews from 1 to selected maximum stars
...
save_to_split_csv(collected_reviews)